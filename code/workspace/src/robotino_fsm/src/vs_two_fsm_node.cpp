...
void robotino_fsm_node::new_goal() {
    ...
    if (!goals.empty() && failed_plans <= 5) {
        ...
        if (done_360) {
            ...
            for (auto goal : goals) {
                ...
                std::cout<<"transformed path time taken: "<<std::chrono::duration_cast<std::chrono::microseconds>(end_clk - start_clk).count()<<std::endl;;
                ...
                std::cout<<"costmap path time taken: "<<std::chrono::duration_cast<std::chrono::microseconds>(end_clk - start_clk).count()<<std::endl;;
    ...
    if (currentPlan.empty()) {
        ...
        publishCurrentPlan();
        publishDefaultPlan();
        publishCurrentPaths(paths);
        publishDefaultPaths(pathsDefault);\
        std::cout<<"\n";
