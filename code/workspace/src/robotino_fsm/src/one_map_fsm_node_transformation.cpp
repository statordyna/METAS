#include "../include/two_map_fsm_node.h"
...
void robotino_fsm_node::mpcStateCallback(const std_msgs::Int8::ConstPtr &msg) {
    ...
    if ((msg->data == REACHED || msg->data == WAITING) && !send_goal) {
        ...
        std::cout<<"\n\n"<<globalRobot.position.x<<", "<<globalRobot.position.y<<", "<<globalRobot.position.z<<std::endl;

        // IF ROBOT STUCK IN KITCHEN (cafe)
        if(dist(std::vector<double>{globalRobot.position.x, globalRobot.position.y, globalRobot.position.z},
                        std::vector<double>{-3.0, 0.4, 0}) <= 1 )
        {
            std::cout<<"\n\n STUCK IN KITCHEN, moving forward.\n"<<std::endl;
            // nav_msgs::Odometry unstuck;
            // unstuck.header.frame_id = "emergency";
            // unstuck.pose.pose = globalRobot;
            // unstuck.pose.pose.position.x = -2.0;
            // unstuck.pose.pose.position.y = 0.5;
            // // unstuck.pose.pose.position.y = -2.7;
    
            // mpcGoalPub.publish(unstuck);
        ...
        // if < than threshold try to recover (replan or move to next goal) otherwise require new goal
        else if (failed < max_fail_before_new_goal) {
            ...
            // /// ROBOT STUCK AT KITCHEN
            // if(
            //     (msg->data == OSCILLATING 
            // &&
            //     failed >= 1 
            // &&
            //     dist(std::vector<double>{globalRobot.position.x, globalRobot.position.y, globalRobot.position.z},
            //          std::vector<double>{(currentPlan.end()-1)->position.x, (currentPlan.end()-1)->position.y,
            //                              (currentPlan.end()-1)->position.z}) < 3.5 // dist of goal is very close
            // // &&
            // // dist(std::vector<double>{globalRobot.position.x, globalRobot.position.y, globalRobot.position.z},
            //     //  std::vector<double>{(currentPlan.end()-1)->position.x, (currentPlan.end()-1)->position.y,
            //                             //  (currentPlan.end()-1)->position.z}) < 0.5) // within a limit
            //     )
            // ||
            //     failed >= 3
            // )
            // {
            //     std::cout<<"\n\nROBOT STUCK, PROBABLY IN KITCHEN.\n"<<std::endl;
            //     stuck_ = 1;
            //     require_new_goal = true;
            //     replan = false;
            //     // filter_last = true;
            ...
            if (msg->data == OSCILLATING &&
                ...
                // send next goal
                // probably prob of location: print the location and check!
...
robotino_fsm_node::robotino_fsm_node(ros::NodeHandle nh, ros::NodeHandle private_nh) : nh_(nh),
    ...
    getCostmapClient = nh_.serviceClient<robotino_fsm::getCostmap_>("/getCostmapSrv_transformation");

    // move_base get plan
    pathReq_client = nh_.serviceClient<robotino_fsm::GetPlan_>(get_plan_topic);
...
void robotino_fsm_node::new_goal() {
    ...
    std::cout<<"\nROBOT POISITION: "<<globalRobot.position.x<<", "<<globalRobot.position.y<<", "<<globalRobot.position.z<<std::endl;
    ROS_INFO_STREAM("is goal empty? " << goals.empty());

    // STUCK IN KITCHEN !!!!
    int filtered_goals_no = 0;
    if(stuck_){
        std::cout<<"\nFILTERING OUT GOALS!!\n"<<std::endl;
        std::cout<<"found "<<goals.size()<<" goals"<<std::endl;
        for(int goal_no = 0; goal_no < goals.size();){
            if(
                dist(std::vector<double>{globalRobot.position.x, globalRobot.position.y, globalRobot.position.z},
                        std::vector<double>{goals[goal_no].position.x, goals[goal_no].position.y,
                                            goals[goal_no].position.z}) < 4            
            )
            {
                goals.erase(goals.begin() + goal_no);
                filtered_goals_no++;
            }
            else
                goal_no++;
    ...
    stuck_ = 0;
    std::cout<<"\nFILTERed GOALS = "<<filtered_goals_no<<"\n"<<std::endl;
...
void robotino_fsm_node::LogCurrentStats(){
    ...
    //path entrpy, avg entrp -> same goal && How many times we're selecting the same goal
    //  
...
robotino_fsm_node::getPlan(const geometry_msgs::Pose end, const geometry_msgs::PosePtr start) {
    ...
    getCostmap_srv.request.getcostmap = true;
    if(getCostmapClient.call(getCostmap_srv))
    {
            // std::cout<<"GOT ITTT\n"<<std::endl;
        
        // getCostmap_srv.response.costarray.data
        ...
        // std::copy(costs_grid, costs_grid + 1000*1000, getPlan.request.costs_grid);
        std::cout<<"\ngetPlan called!\n";
        // for(int i = 0; i< 1000*1000; i++)
        //     if(getPlan.request.costs_grid[i] > 0 ) std::cout<<getPlan.request.costs_grid[i]<<" ";
